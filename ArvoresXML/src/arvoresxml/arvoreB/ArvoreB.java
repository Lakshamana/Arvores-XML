...
public class ArvoreB {
    ...
    //Getters e Setters dos atributos iElem,order e root
    public int getiElem() {
        return iElem;
    }
    ...
    public void insere(int k) {
        ...
        if (BuscaChave(root, k) == null) { //só insere se não houver, para evitar duplicação de chaves
            //verifica se a chave está vazia
            if (root.getN() == 0) {
                root.getChave().set(0, k);//seta a chave na primeira posição da root
                root.setN(root.getN() + 1);
            } else { //caso nao estaja vazia
                No r = root;
                //verifica se a root está cheia
                if (r.getN() == order - 1) {//há necessidade de dividir a root
                    No s = new No(order);
                    root = s;
                    s.setFolha(false);
                    s.setN(0);
                    ...
                    insereNoNaoCheio(s, k);//depois de dividir a root começa inserindo apartir da root

                } else {//caso contrario começa inserindo apartir da root
                    insereNoNaoCheio(r, k);
                }
            }
            iElem++;//incrementa o numero de elemantos na arvore
        }
    }
    ...
    public void insereNoNaoCheio(No x, int k) {
        ...
        if (x.eFolha()) {
            ...
            if ((x.getFilho().get(i)).getN() == order - 1) {
    ...
    public No BuscaChave(No X, int k) {
        ...
        if (X.eFolha()) { //se o no X for folha
            return null;
        } else {
    ...
    public void Remove(int k) {
        ...
        if (BuscaChave(this.root, k) != null) {
            //N é o nó onde se encontra k
            No N = BuscaChave(this.root, k);
            int i = 1;
            ...
            if (N.eFolha()) {
                ...
                No S = Antecessor(this.root, k);//S eh onde se encontra o antecessor de k
            ...
            iElem--;//decrementa o numero de elementos na arvoreB
        }
    }
    ...
    private void Balanceia_Folha(No F) {
        ...
        if (F.getN() < Math.floor((order - 1) / 2)) {
            No P = getPai(root, F);//P é o pai de F
            int j = 1;
            ...
            if (j == 1 || (P.getFilho().get(j - 2)).getN() == Math.floor((order - 1) / 2)) {
                //verifica se o irmão à direita de F não tem chaves para emprestar
                if (j == P.getN() + 1 || (P.getFilho().get(j).getN() == Math.floor((order - 1) / 2))) {
    ...
    private void Diminui_Altura(No X) {
        ...
        //verifica se X é a root
        if (X == this.root) {
            //verifica se X está vazio
            if (X.getN() == 0) {
                this.root = X.getFilho().get(0);//o filho o de x passa a ser root
                X.getFilho().set(0, null); // desaloca o filho de x
            }
        } else {//caso contrario(X nao é root)
            int t = (int) Math.floor((order - 1) / 2);
            //verifica se o numero de chaves de X é menor que o permitido
            if (X.getN() < t) {
                P = getPai(root, X);//P é pai de X
                j = 1;
                ...
                //junta os nós
                if (j > 1) {
                    Juncao_No(getPai(root, X), j - 1);
                } else {
                    Juncao_No(getPai(root, X), j);
                }
                Diminui_Altura(getPai(root, X));
            }
        }
    }
    ...
    private void Balanceia_Esq_Dir(No P, int e, No Esq, No Dir) {
        ...
        if (!Dir.eFolha()) {//nao foi testado, mas teoricamente funciona
            for (int i = 0; i > Dir.getN(); i++) {
    ...
    //Parâmetros: N - Nó onde se deve iniciar a varredura, order - Nova order da arvoreB
    public void LimparArvore(No N, int order) {

        for (int i = 0; i < N.getN() + 1; i++) {
            if (!N.eFolha()) {
                LimparArvore(N.getFilho().get(i), order);
            }
